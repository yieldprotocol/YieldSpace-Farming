...
import "./../external/compound/CTokenInterfaces.sol";


/// @dev The CPool contract exchanges Compound's cDai for fyDai at a price defined by a specific formula.
contract CPool is IPool, Delegable(), ERC20Permit {
    ...
    CTokenInterface public override cDai;
    IFYDai public override fyDai;

    constructor(address cDai_, address fyDai_, string memory name_, string memory symbol_)
        public
        ERC20Permit(name_, symbol_)
    {
        cDai = CTokenInterface(cDai_);
        fyDai = IFYDai(fyDai_);
    ...
    function mantissaTo6464(uint256 x) pure view returns(int128) {
        require(x < type(uint196).max, "CPool: Mantissa conversion overflow");
        return toUint128((x << 64) / 10**27); // TODO: Does `exchangeRateCurrent` use 27 or 28 decimals?
    }
    ...
    /// @param cDaiIn The initial Dai liquidity to provide.
    ...
    function sellDai(address from, address to, uint128 cDaiIn)
        external override
        onlyHolderOrDelegate(from, "Pool: Only Holder Or Delegate")
        returns(uint128)
    {
        uint128 fyDaiOut = sellDaiPreview(cDaiIn);
    ...
    /// @dev Returns how much fyDai would be obtained by selling `cDaiIn` cDai
    /// @param cDaiIn Amount of cDai hypothetically sold.
    /// @return Amount of fyDai hypothetically bought.
    function sellDaiPreview(uint128 cDaiIn)
        public override
        beforeMaturity
        returns(uint128)
    {
        uint128 cDaiReserves = getDaiReserves();
        ...
        uint128 fyDaiOut = YieldMath.fyDaiOutForDaiIn(
            ...
            k,
            g1,
            mantissaTo6464(cDai.exchangeRateCurrent()) // TODO: Can be exchangeRateStored for a `view` function
        );
    ...
    function buyDai(address from, address to, uint128 cDaiOut)
        external override
        onlyHolderOrDelegate(from, "Pool: Only Holder Or Delegate")
        returns(uint128)
    {
        uint128 fyDaiIn = buyDaiPreview(cDaiOut);
    ...
    /// @dev Returns how much fyDai would be required to buy `cDaiOut` cDai.
    /// @param cDaiOut Amount of cDai hypothetically desired.
    /// @return Amount of fyDai hypothetically required.
    function buyDaiPreview(uint128 cDaiOut)
        public override
        beforeMaturity
        returns(uint128)
    {
        return YieldMath.fyDaiInForDaiOut(
            ...
            k,
            g2,
            mantissaTo6464(cDai.exchangeRateCurrent())
        );
    }
    ...
    function sellFYDaiPreview(uint128 fyDaiIn)
        ...
        returns(uint128)
    {
        return YieldMath.cDaiOutForFYDaiIn(
            getDaiReserves(),
            getFYDaiReserves(),
            fyDaiIn,
            toUint128(maturity - now), // This can't be called after maturity
        ...
        returns(uint128)
    {
        ...
        uint128 cDaiIn = YieldMath.cDaiInForFYDaiOut(
            cDaiReserves,
            fyDaiReserves,
            fyDaiOut,
            toUint128(maturity - now), // This can't be called after maturity
